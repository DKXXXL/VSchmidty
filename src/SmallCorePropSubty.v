...
Theorem eq_ty_dec:
    ...
    right; intro. inversion H; subst; try contradiction.
    ...
    destruct (IHT1 T'1); destruct (IHT2 T'2); destruct (eq_id_dec f f0);
    subst; eauto;
    try (left; eauto; fail);
...
Theorem subty_refl_eq:
    ...
    rewrite IHh1; eauto. rewrite IHh2; eauto.
    assert (subty q1 (TRcons i p1 q1)). eapply subTrans; eauto.
    destruct (subty_trcons_never_rec _ _ _ H0 H5). 
    
    assert (subty q1 (TRcons i p q1)). eapply subTrans; eauto.
    destruct (subty_trcons_never_rec _ _ _ H0 H5). 
...
Theorem subty_dec_compl:
    ...
    try (
        match goal with
        | |- {subty (TVar ?x) (TVar ?y) /\ subty (TVar _) (TVar _)} + {_} + {_} + {_} =>
            destruct (eq_id_dec x y); subst; eauto;
            try (fst; eauto; fail);
            try (fth; split; intros hhhh; pose (subty_onlyrefl_TVar0 _ _ hhhh) as hhhhh; inversion hhhhh; subst; try contradiction; fail)
        | |- {subty (TVar ?x) _ /\ subty _ _} + {_} + {_} + {_} =>
            try (fth; 
            split; intros hhhh; [pose (subty_onlyrefl_TVar0 _ _ hhhh) as hhhhh | pose (subty_onlyrefl_TVar1 _ _ hhhh) as hhhhh] ; inversion hhhhh; subst; try contradiction; fail
            )
        | |- {subty _ (TVar ?x) /\ subty _ _} + {_} + {_} + {_} =>
            try (fth; 
            split; intros hhhh; [pose (subty_onlyrefl_TVar1 _ _ hhhh) as hhhhh | pose (subty_onlyrefl_TVar0 _ _ hhhh) as hhhhh] ; inversion hhhhh; subst; try contradiction; fail
            )
        end; fail
    )
    .
    ...
    (* case TRcons *)
    destruct (wf_ty_dec (TRcons f T2_1 T2_2)).
    ...
    destruct (wf_ty_dec (TRcons f T1_1 T1_2)).
        ...
        | h0 : subty ?x0 ?y0, h1: subty ?x1 ?y1 |-
            ...
            try (
                    ...
                    | hhhh0 : wf_ty x0,
                      ...
                      hhhh2 : wf_ty x1,
                      hhhh3 : only_rcd x1  |- _ =>
                        eapply (subRcdd _ _ _ _ hhhh0 hhhh1 hhhh2 h0 h1)
                    end
                    )
        | h1 : subty ?y0 (TRcons ?i0 ?x1 ?y1) |-
            ...
            assert (only_rcd y0); eauto 3;clear_dupli;
                ...
                | hh0 : wf_ty y0,
                  ...
                  hh4 : wf_ty x0
                    |- _ =>            
                    eapply (subRcdw _ _ _ _ hh0 hh1 hh2 hh3 h1 hh4)
                end
        | |- ~ _ =>
            intro hh0; 
    ...
    destruct (eq_id_dec f f0).
    poses' (IHT1_1 T2_1);
    poses' (IHT1_2 T2_2);
    poses' (IHT1_2 (TRcons f0 T2_1 T2_2));
    subst; eauto;
    ...
    snd; split; try general_process. eapply subRcdw; eauto.
    trd; split; try general_process. eapply subRcdw; eauto.
